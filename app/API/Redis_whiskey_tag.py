...
class Find_or_Update():
	...
	def __init__(self, keyword):
		...
		#	Redis 連線設定
		pool = redis.ConnectionPool(host='自建立kafka server', port=6379, db=1, decode_responses=True)
...
class editor():
	...
	def edit_reply(self):
		self.data['footer']['contents'][0]['action']['text'] = '評論'+self.dbdata['name']
...
def tag_api(keyword):
	...
	with open('./app/API/whiskey_tag_example.txt', 'r', encoding='windows-1252') as tag_file:
		...
		with open('./app/API/whiskey_tag_output.json', 'w', encoding='utf-8') as f2:
